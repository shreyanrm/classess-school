...
| `app/events.py` | Emits `coaching.signal_generated` (**always private + teacher-first**), `quality.review_signed_off` (requires a human ref), and `continuity.handover_recorded` on the attributed, append-only event envelope (`growth` purpose). Opaque ids only. |
