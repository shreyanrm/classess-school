# Classess 3.0 — Brand System (system of record)

**Open `classess-3.0.html`.** It is the single unified source — the brand kit
(identity, colour, type, depth, laws), the full motion language, and the complete
component library, composed as one editorial document. It supersedes all prior
versions, including the standalone brand-kit / motion-lab / playground files,
which remain only as the working sources it was assembled from.

Stylesheets load in order: tokens.css → components.css → playground.css → classess-3.css
Scripts load in order: playground.js → classess-3.js
Fonts are CDN-linked for review; self-host for production (see FONTS.md).
