...
| File | Responsibility |
| --- | --- |
| `app/ingest.py` | Ingests curriculum from documents / standards / publisher content into the ontology graph. The **document-understanding** step has a **real Gemini-backed path** (`app/gemini.py`, selected automatically when the Gemini key is set) plus a structured-source path and the `NullDocumentUnderstanding` fallback. It **degrades gracefully**: with no key, ingestion uses the structured-source path or records **pending extraction** — it never invents extraction output. Maps each outline node onto its typed ontology table under its parent, resolves model-proposed prerequisite hints into **UNCONFIRMED** edges on the snapshot, and applies the confidence gate (a node below it is flagged `needs_review`). Deterministic, idempotent node ids. |
| `app/gemini.py` | The **real** Gemini document-understanding client. Given curriculum text/markdown/PDF-text it calls Gemini `generateContent` over HTTPS via **httpx**, reading the provider key **by NAME** (`clss.aifabric.dev.gemini_api_key` → `CLSS_AIFABRIC_DEV_GEMINI_API_KEY`) only at the point of egress — never hardcoded, never logged, never returned. Parses the model's strict JSON into ontology-typed outline nodes + **proposed** prerequisite edge hints (capped below certainty). **Degrades cleanly** with no key (clearly-marked unavailable, no fabrication); the deterministic parser then runs. Import-safe and offline-testable (injectable httpx transport). |
...
| `clss.ontology.dev.doc_understanding_key` | `CLSS_ONTOLOGY_DEV_DOC_UNDERSTANDING_KEY` | The gateway-routed document-understanding provider seam. Unset → draft / structured-source only. |
| `clss.aifabric.dev.gemini_api_key` | `CLSS_AIFABRIC_DEV_GEMINI_API_KEY` | The **real** Gemini document-understanding key (shared AI-fabric namespace), read by NAME at egress in `app/gemini.py`. Set → live curriculum extraction; unset → the deterministic parser path. |
