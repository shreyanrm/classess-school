...
| `app/ptm.py` | The **parent–teacher meeting** lifecycle: **book** (`request_booking` → `confirm_booking`, permission-gated — proposed until a human confirms; reminders only for a confirmed slot), **prepare** (`prepare`: a plain-language child brief + pre-submitted questions **screened** before they reach the teacher; a flagged question is a safeguarding matter, never silently routed), and **follow up** (`follow_up`: a shared plan reusing the silent assistant's proposed action items with the **parent's part highlighted**, assigned only by a human). |
| `app/translation.py` | The **multilingual + code-switching** interface. `render_for_reader` translates to a **reader's preferred language** (absent preference → intact passthrough, never guessed). Preserves protected subject terminology verbatim (`mask`/`restore` round-trip), detects and preserves code-switch spans, and degrades to a **content-preserving pass-through** that never drops or garbles text and never sends text off-box without the gateway. |
| `app/events.py` | Emits `message.sent`, `meeting.scheduled`, `sentiment.observed`, `safeguarding.escalated`, `ptm.scheduled`, `ptm.completed`, and `action.assigned` on the attributed, append-only envelope. Opaque ids only — **never the message body**; `message.sent` refuses an unscreened message, `ptm.scheduled` refuses an unapproved booking, `action.assigned` refuses an unowned/auto-assigned action; a safeguarding escalation rides the `intervention` purpose. |
