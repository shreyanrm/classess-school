...
if (tab) { await tab.dispatchEvent('click'); await p.waitForTimeout(1800); }
const el = await p.$('.scatter');
if (el) {
  await el.scrollIntoViewIfNeeded();
  await p.waitForTimeout(700);
  const card = await p.evaluateHandle(() => document.querySelector('.scatter')?.closest('.viz-card'));
  await (card.asElement() ?? el).screenshot({ path: `${OUT}/scatter-zoom.png` });
  console.log('shot scatter-zoom');
} else console.log('no .scatter found');
await b.close();
